...
//const API_URL = 'https://fixmaclk-1-0.onrender.com/api/products';
 const API_URL = 'https://fixmaclk-1-0.onrender.com/api/products';
const CLICKED_PRODUCTS_API_URL = 'https://fixmaclk-1-0.onrender.com/api/clicked-products';
...
const PAYMENTS_API_URLwithItemcode = 'https://fixmaclk-1-0.onrender.com/api/payments/with-itemcodes';
...
const ProductList = ({ darkMode }) => {
  ...
  const loadSupplierQuantities = async () => {
    ...
    try {
      const res = await fetch('https://fixmaclk-1-0.onrender.com/api/suppliers');
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      ...
      const response = await fetch('https://fixmaclk-1-0.onrender.com/api/product-uploads/bulk-upload',
      {
        method: 'POST',
  ...
  const fetchUploadedRecords = async (page = 1) => {
    ...
    setUploadedError('');
    try {
      const res = await fetch(`https://fixmaclk-1-0.onrender.com/api/product-uploads?page=${page}&limit=20`);
  ...
  const handleExportAllUploaded = async () => {
    try {
      const res = await fetch('https://fixmaclk-1-0.onrender.com/api/product-uploads/export-all');
